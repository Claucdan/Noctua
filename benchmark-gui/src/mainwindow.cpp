...
MainWindow::MainWindow(QWidget* parent)
    : QMainWindow(parent)
    , m_runner(std::make_unique<BenchmarkRunner>(this))
    , m_updateTimer(new QTimer(this))
{
    setupUi();

    // Connect runner signals
    connect(m_runner.get(), &BenchmarkRunner::statsUpdated,
            this, &MainWindow::onStatsUpdated);
    connect(m_runner.get(), &BenchmarkRunner::benchmarkStarted,
            this, &MainWindow::onBenchmarkStarted);
    connect(m_runner.get(), &BenchmarkRunner::benchmarkStopped,
            this, &MainWindow::onBenchmarkStopped);
    connect(m_runner.get(), &BenchmarkRunner::errorOccurred,
