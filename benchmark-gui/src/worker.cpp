...
Worker::Worker(
        const QString& host, quint16 port, const QString& topicName, uint32_t partitionId, int qps, QObject* parent)
    : QObject(parent)
    , m_host(host)
    , m_port(port)
...
void Worker::updateStats(uint64_t latencyMicros, bool success) {
  ...
  if (latencyMicros > 0) {
    ...
    if (m_stats.latencySampleCount == 1) {
      ...
      m_stats.minLatencyMs = std::min(m_stats.minLatencyMs, static_cast<double>(latencyMicros) / 1000.0);
      m_stats.maxLatencyMs = std::max(m_stats.maxLatencyMs, static_cast<double>(latencyMicros) / 1000.0);
    }

    m_stats.avgLatencyMs =
            static_cast<double>(m_stats.latencyTotalMicros) / static_cast<double>(m_stats.latencySampleCount) / 1000.0;
...
void Worker::processResponse(const QByteArray& data) {
  ...
  const bool success = response.valid && response.errorCode == noctua::rpc::error_code_t::OK
