...
void BenchmarkRunner::createWorkers() {
  ...
  for (int i = 0; i < m_config.numWriters; ++i) {
    ...
    auto* worker = new WriterWorker(
            m_config.host, m_config.port, m_config.topicName, partitionId, m_config.writerQps, m_config.messageSize);

    connect(worker, &Worker::statsUpdated, this, &BenchmarkRunner::onWorkerStatsUpdated);
    connect(worker, &Worker::errorOccurred, this, &BenchmarkRunner::errorOccurred);
  ...
  for (int i = 0; i < m_config.numReaders; ++i) {
    auto* thread = new QThread(this);
    const auto partitionId = randomPartitionForWorker();
    auto* worker = new ReaderWorker(m_config.host, m_config.port, m_config.topicName, partitionId, m_config.readerQps);
...
bool BenchmarkRunner::initializeTopic(QString* errorMessage) const {
  ...
  if (!socket.waitForConnected(5000)) {
    if (errorMessage != nullptr) {
      *errorMessage = QString("Failed to connect for topic initialization: %1").arg(socket.errorString());
    }
    return false;
  }
  ...
  const auto pushRequest = ProtocolUtils::createPushRequest(m_config.topicName.toUtf8(), initPartition, initMessage);
  if (socket.write(pushRequest) != pushRequest.size() || !socket.waitForBytesWritten(5000)) {
    if (errorMessage != nullptr) {
      *errorMessage = QString("Failed to write initialization PUSH: %1").arg(socket.errorString());
    }
    return false;
  }
  ...
  if (!pushResponse.valid || pushResponse.errorCode != ProtocolUtils::ErrorCode::OK
      || pushResponse.opcode != ProtocolUtils::Opcode::PUSH) {
    if (errorMessage != nullptr) {
      *errorMessage = QString("Initialization PUSH failed: %1").arg(QString::fromUtf8(pushResponse.message));
    }
    return false;
  }
  ...
  if (socket.write(deleteRequest) != deleteRequest.size() || !socket.waitForBytesWritten(5000)) {
    if (errorMessage != nullptr) {
      *errorMessage = QString("Failed to write initialization DELETE: %1").arg(socket.errorString());
    }
    return false;
  }
  ...
  if (!deleteResponse.valid || deleteResponse.errorCode != ProtocolUtils::ErrorCode::OK
      || deleteResponse.opcode != ProtocolUtils::Opcode::DELETE) {
    if (errorMessage != nullptr) {
      *errorMessage = QString("Initialization DELETE failed: %1").arg(QString::fromUtf8(deleteResponse.message));
    }
    return false;
  }
...
bool BenchmarkRunner::readResponse(QTcpSocket& socket, ProtocolUtils::Response* response, QString* errorMessage) {
  QByteArray buffer;

  while (buffer.size() < static_cast<int>(sizeof(noctua::rpc::response_header_t))) {
    if (!socket.waitForReadyRead(5000)) {
      if (errorMessage != nullptr) {
        *errorMessage = QString("Timed out waiting for response header: %1").arg(socket.errorString());
      }
      return false;
    }
  ...
  while (buffer.size() < static_cast<int>(totalLength)) {
    if (!socket.waitForReadyRead(5000)) {
      if (errorMessage != nullptr) {
        *errorMessage = QString("Timed out waiting for full response: %1").arg(socket.errorString());
      }
      return false;
    }
    buffer.append(socket.readAll());
...
BenchmarkRunner::AggregatedStats BenchmarkRunner::buildAggregatedStatsLocked() const {
  ...
  const auto elapsedMicros = std::chrono::duration_cast<std::chrono::microseconds>(now - m_startTime).count();
  if (elapsedMicros > 0) {
...
double BenchmarkRunner::averageLatencyMs(const LatencyAccumulator& accumulator) {
  ...
  return static_cast<double>(accumulator.totalMicros) / static_cast<double>(accumulator.sampleCount) / 1000.0;
}
