...
void BenchmarkGuiQtTest::benchmarkRunner_aggregatesStatsFromAllWorkers() {
  ...
  QVERIFY(QObject::connect(
          &writer1, SIGNAL(statsUpdated(Worker::Stats)), &runner, SLOT(onWorkerStatsUpdated(Worker::Stats))));
  QVERIFY(QObject::connect(
          &writer2, SIGNAL(statsUpdated(Worker::Stats)), &runner, SLOT(onWorkerStatsUpdated(Worker::Stats))));
  QVERIFY(QObject::connect(
          &reader1, SIGNAL(statsUpdated(Worker::Stats)), &runner, SLOT(onWorkerStatsUpdated(Worker::Stats))));
